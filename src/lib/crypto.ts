...
export async function encryptPayload(
  plainText: string,
  passphrase: string = 'DHRISTI_SEOC_SECRET_2026'
): Promise<{ ciphertext: string; iv: string; verificationHash: string }> {
  try {
...
export function decryptPayload(ciphertext: string, fallbackPlain?: string): string {
